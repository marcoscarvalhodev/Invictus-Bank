...
const RewardsHero = ({ smallState }: RewardsProps) => {
  ...
  const [rangeState, setRangeState] = React.useState(0)

  const handleDrag : React.ChangeEventHandler<HTMLInputElement> = (event) => {

    const range = event.target as HTMLInputElement
    setRangeState(Number(range.value))
  }
  ...
  return (
    <StyledRewardsHero className='container'>
      ...
      <input type="number" name='range-lid'  onChange={handleDrag}/>
      </div>

      <div className='flex-hero flex-hero-2'>
        <RewardsGift rangeState={rangeState}/>
      </div>
    </StyledRewardsHero>
  );
};
